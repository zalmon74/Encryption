...
Alphabet::Alphabet(const std::map<size_t, std::string>& input_map)
{
  ...
  if (input_map.size() != 0)
  {
    for (auto& el : input_map)
    {
      ...
      if (el.first >= NUM_DIVISION_SYMBOLS)
      {
      ...
      {
        if (this->ind_start_uppercase == UNDEFINED_VALUE_SIZE_T)
          this->ind_start_uppercase = el.first;
  ...
  this->size_alphabet = std::ceil(this->alph_ind_symb.size()/2.0);
  if (this->alph_ind_symb.size() != 0) 
    this->f_dummy = false;
}
